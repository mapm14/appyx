...
abstract class Node(
    ...
    private fun HandleBackPress() {
        val backPressHandlerPlugins = remember { plugins.filterIsInstance<BackPressHandler>() }
        ...
        DisposableEffect(dispatcher) {
            backPressHandlerPlugins.forEach { plugin ->
                ...
                plugin.onBackPressedCallbacks.forEach { dispatcher?.addCallback(it) }
            }
            onDispose {
                backPressHandlerPlugins.forEach { plugin ->
                    plugin.onBackPressedCallbacks.forEach { it.remove() }
                }
            }
        }
    }
    ...
    private fun BackPressHandler.isCorrect(): Boolean =
        onBackPressedCallback != null && listOf(onBackPressedCallback) == onBackPressedCallbacks ||
